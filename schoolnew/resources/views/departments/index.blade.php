            ...
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        @endcan
                        <a href="{{ route('admin.departments.trash') }}" class="btn btn-outline-danger position-relative">
                                        ...
                                        <div class="common-align gap-2 justify-content-start">
                                            ...
                                            @can('delete staff')
                                                <form action="{{ route('admin.departments.destroy', $department) }}" method="POST" class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="square-white trash-7 border-0 bg-transparent p-0 move-to-trash" title="Move to Trash" data-name="{{ $department->name }}">
...
jQuery(document).ready(function() {
    ...
    bulkDeleteBtn.on('click', function() {
        ...
        Swal.fire({
            title: 'Move to Trash?',
            html: `You are about to move <strong>${selectedIds.length}</strong> department(s) to trash:<br><br><small>${namesText}</small><br><br><small class="text-muted">You can restore them later from the trash.</small>`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                jQuery.ajax({
                    url: '{{ route("admin.departments.bulk-delete") }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                    ...
                    beforeSend: function() {
                        Swal.fire({
                            ...
                            text: 'Please wait while we move the selected departments to trash.',
    ...
    jQuery(document).on('click', '.move-to-trash', function(e) {
        ...
        var itemName = jQuery(this).data('name') || 'this department';

        Swal.fire({
            title: 'Move to Trash?',
            html: `You are about to move <strong>${itemName}</strong> to trash.<br><small class="text-muted">You can restore this department later from the trash.</small>`,
            icon: 'warning',
            showCancelButton: true,
