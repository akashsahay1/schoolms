...
@section('title', 'Departments Trash')

@section('page-title', 'Departments Trash')
		...
		<div class="card">
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					<h5>Deleted Departments ({{ $trashedCount }})</h5>
					<div class="d-flex gap-2">
						...
						<a href="{{ route('admin.departments.index') }}" class="btn btn-outline-primary">
							<i data-feather="arrow-left" class="me-1"></i> Back to Departments
						</a>
					</div>
				</div>
			</div>
			<div class="card-body">
				<!-- Search -->
				<form method="GET" action="{{ route('admin.departments.trash') }}" class="mb-4">
					<div class="row g-3">
						<div class="col-md-4">
						...
						@if(request('search'))
							<div class="col-md-1">
								<a href="{{ route('admin.departments.trash') }}" class="btn btn-outline-secondary w-100" title="Clear">
									<i data-feather="x"></i>
								</a>
							</div>
						@endif
					...
					<table class="table table-striped table-hover">
						...
						<tbody>
							@forelse($departments as $department)
								<tr>
									<td>
										<input type="checkbox" class="form-check-input department-checkbox" value="{{ $department->id }}" data-name="{{ $department->name }}">
									</td>
									<td>{{ $departments->firstItem() + $loop->index }}</td>
									<td>{{ $department->name }}</td>
									<td>{{ Str::limit($department->description, 50) ?? '-' }}</td>
									<td>
										<span class="badge badge-light-primary">{{ $department->staff_count ?? 0 }}</span>
									</td>
									<td>
										<span class="text-muted" title="{{ $department->deleted_at->format('d M Y, h:i A') }}">
									...
									<td>
										<div class="common-align gap-2 justify-content-start">
											...
											<button type="button" class="square-white border-0 bg-transparent p-0 force-delete-btn" title="Delete Permanently" data-id="{{ $department->id }}" data-name="{{ $department->name }}">
...
<form id="emptyTrashForm" action="{{ route('admin.departments.empty-trash') }}" method="POST" class="d-none">
	@csrf
	@method('DELETE')
</form>
@endsection
...
jQuery(document).ready(function() {
	...
	const departmentCheckboxes = jQuery('.department-checkbox');
	...
	function updateBulkState() {
		const checkedCount = jQuery('.department-checkbox:checked').length;
		...
		const totalCheckboxes = departmentCheckboxes.length;
	...
	selectAllCheckbox.on('change', function() {
		departmentCheckboxes.prop('checked', jQuery(this).is(':checked'));
		updateBulkState();
	});

	// Individual checkbox handler
	departmentCheckboxes.on('change', function() {
		updateBulkState();
	});
	...
	function getSelectedData() {
		...
		jQuery('.department-checkbox:checked').each(function() {
	...
	bulkRestoreBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `You are about to restore <strong>${ids.length}</strong> department(s):<br><br><small>${namesText}</small>`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.departments.bulk-restore") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						department_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Restoring...',
							text: 'Please wait while we restore the selected departments.',
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>${ids.length}</strong> department(s):<br><br><small>${namesText}</small>`,
			icon: 'warning',
			showCancelButton: true,
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.departments.bulk-force-delete") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						department_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Deleting...',
							text: 'Please wait while we permanently delete the selected departments.',
							allowOutsideClick: false,
							allowEscapeKey: false,
							didOpen: () => {
								Swal.showLoading();
	...
	jQuery('#emptyTrashBtn').on('click', function() {
		Swal.fire({
			title: 'Empty Trash?',
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>all {{ $trashedCount }} department(s)</strong> in the trash.`,
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d33',
			cancelButtonColor: '#6c757d',
