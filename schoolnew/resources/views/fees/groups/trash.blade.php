...
@section('title', 'Fee Groups Trash')

@section('page-title', 'Fee Groups Trash')
		...
		<div class="card">
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					<h5>Deleted Fee Groups ({{ $trashedCount }})</h5>
					<div class="d-flex gap-2">
						...
						@endif
						<a href="{{ route('admin.fees.groups.index') }}" class="btn btn-outline-primary">
							<i data-feather="arrow-left" class="me-1"></i> Back to Fee Groups
						</a>
					</div>
				</div>
			</div>
			<div class="card-body">
				<form method="GET" action="{{ route('admin.fees.groups.trash') }}" class="mb-4">
					<div class="row g-3">
						<div class="col-md-4">
						...
						@if(request('search'))
							<div class="col-md-1">
								<a href="{{ route('admin.fees.groups.trash') }}" class="btn btn-outline-secondary w-100" title="Clear">
									<i data-feather="x"></i>
								</a>
							</div>
						@endif
						...
						<tbody>
							@forelse($feeGroups as $feeGroup)
								<tr>
									...
									<td>
										<span class="text-muted" title="{{ $feeGroup->deleted_at->format('d M Y, h:i A') }}">
											{{ $feeGroup->deleted_at->diffForHumans() }}
										</span>
									</td>
									<td>
										<div class="common-align gap-2 justify-content-start">
											<form action="{{ route('admin.fees.groups.restore', $feeGroup->id) }}" method="POST" class="d-inline">
												@csrf
											...
											<button type="button" class="square-white border-0 bg-transparent p-0 force-delete-btn" title="Delete Permanently" data-id="{{ $feeGroup->id }}" data-name="{{ $feeGroup->name }}">
				...
				<div class="d-flex justify-content-center mt-4">
					{{ $feeGroups->withQueryString()->links() }}
				</div>
			</div>
		</div>
	</div>
...
<form id="emptyTrashForm" action="{{ route('admin.fees.groups.empty-trash') }}" method="POST" class="d-none">
	@csrf
	@method('DELETE')
</form>
@endsection
...
jQuery(document).ready(function() {
	...
	bulkRestoreBtn.on('click', function() {
		...
		Swal.fire({
			title: 'Restore Fee Groups?',
			html: `You are about to restore <strong>${ids.length}</strong> fee group(s):<br><br><small>${namesText}</small>`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.fees.groups.bulk-restore") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						fee_group_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Restoring...',
							text: 'Please wait while we restore the selected fee groups.',
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>${ids.length}</strong> fee group(s):<br><br><small>${namesText}</small>`,
			icon: 'warning',
			showCancelButton: true,
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.fees.groups.bulk-force-delete") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						fee_group_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Deleting...',
							text: 'Please wait while we permanently delete the selected fee groups.',
							allowOutsideClick: false,
							allowEscapeKey: false,
							didOpen: () => {
								Swal.showLoading();
	...
	jQuery('.force-delete-btn').on('click', function() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: `/admin/fees/groups/${id}/force-delete`,
					type: 'DELETE',
					data: {
						_token: '{{ csrf_token() }}'
					},
	...
	jQuery('#emptyTrashBtn').on('click', function() {
		Swal.fire({
			title: 'Empty Trash?',
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>all {{ $trashedCount }} fee group(s)</strong> in the trash.`,
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d33',
			cancelButtonColor: '#6c757d',
