...
@section('title', 'Designations Trash')

@section('page-title', 'Designations Trash')
		...
		<div class="card">
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					<h5>Deleted Designations ({{ $trashedCount }})</h5>
					<div class="d-flex gap-2">
						...
						@endif
						<a href="{{ route('admin.designations.index') }}" class="btn btn-outline-primary">
							<i data-feather="arrow-left" class="me-1"></i> Back to Designations
						</a>
					</div>
				</div>
			</div>
			<div class="card-body">
				<!-- Search -->
				<form method="GET" action="{{ route('admin.designations.trash') }}" class="mb-4">
					<div class="row g-3">
						<div class="col-md-4">
						...
						@if(request('search'))
							<div class="col-md-1">
								<a href="{{ route('admin.designations.trash') }}" class="btn btn-outline-secondary w-100" title="Clear">
									<i data-feather="x"></i>
								</a>
							</div>
						@endif
						...
						<tbody>
							@forelse($designations as $designation)
								<tr>
									...
									<td>
										<span class="badge badge-light-primary">{{ $designation->staff_count ?? 0 }}</span>
									</td>
									<td>
										<span class="text-muted" title="{{ $designation->deleted_at->format('d M Y, h:i A') }}">
											{{ $designation->deleted_at->diffForHumans() }}
										</span>
									</td>
									<td>
										<div class="common-align gap-2 justify-content-start">
											<form action="{{ route('admin.designations.restore', $designation->id) }}" method="POST" class="d-inline">
												@csrf
											...
											<button type="button" class="square-white border-0 bg-transparent p-0 force-delete-btn" title="Delete Permanently" data-id="{{ $designation->id }}" data-name="{{ $designation->name }}">
...
<form id="emptyTrashForm" action="{{ route('admin.designations.empty-trash') }}" method="POST" class="d-none">
	@csrf
	@method('DELETE')
</form>
@endsection
...
jQuery(document).ready(function() {
	...
	bulkRestoreBtn.on('click', function() {
		...
		Swal.fire({
			title: 'Restore Designations?',
			html: `You are about to restore <strong>${ids.length}</strong> designation(s):<br><br><small>${namesText}</small>`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.designations.bulk-restore") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						designation_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Restoring...',
							text: 'Please wait while we restore the selected designations.',
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>${ids.length}</strong> designation(s):<br><br><small>${namesText}</small>`,
			icon: 'warning',
			showCancelButton: true,
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.designations.bulk-force-delete") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						designation_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Deleting...',
							text: 'Please wait while we permanently delete the selected designations.',
							allowOutsideClick: false,
							allowEscapeKey: false,
							didOpen: () => {
								Swal.showLoading();
	...
	jQuery('.force-delete-btn').on('click', function() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.designations.force-delete", "") }}/' + id,
					type: 'DELETE',
					data: {
						_token: '{{ csrf_token() }}'
					},
	...
	jQuery('#emptyTrashBtn').on('click', function() {
		Swal.fire({
			title: 'Empty Trash?',
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>all {{ $trashedCount }} designation(s)</strong> in the trash.`,
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d33',
			cancelButtonColor: '#6c757d',
