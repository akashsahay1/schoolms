...
@section('title', 'Parents Trash')

@section('page-title', 'Parents Trash')
		...
		<div class="card">
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					<h5>Deleted Parents ({{ $trashedCount }})</h5>
					<div class="d-flex gap-2">
						...
						@endif
						<a href="{{ route('admin.parents.index') }}" class="btn btn-outline-primary">
							<i data-feather="arrow-left" class="me-1"></i> Back to Parents
						</a>
					</div>
				</div>
			</div>
			<div class="card-body">
				<!-- Search -->
				<form method="GET" action="{{ route('admin.parents.trash') }}" class="mb-4">
					<div class="row g-3">
						<div class="col-md-4">
						...
						@if(request('search'))
							<div class="col-md-1">
								<a href="{{ route('admin.parents.trash') }}" class="btn btn-outline-secondary w-100" title="Clear">
									<i data-feather="x"></i>
								</a>
							</div>
						@endif
						...
						<tbody>
							@forelse($parents as $parent)
								<tr>
									...
									<td>
										<span class="text-muted" title="{{ $parent->deleted_at->format('d M Y, h:i A') }}">
											{{ $parent->deleted_at->diffForHumans() }}
										</span>
									</td>
									<td>
										<div class="common-align gap-2 justify-content-start">
											<form action="{{ route('admin.parents.restore', $parent->id) }}" method="POST" class="d-inline">
												@csrf
											...
											<button type="button" class="square-white border-0 bg-transparent p-0 force-delete-btn" title="Delete Permanently" data-id="{{ $parent->id }}" data-name="{{ $parent->father_name ?? $parent->mother_name ?? $parent->guardian_name ?? 'Parent' }}">
...
<form id="emptyTrashForm" action="{{ route('admin.parents.empty-trash') }}" method="POST" class="d-none">
	@csrf
	@method('DELETE')
</form>
@endsection
...
jQuery(document).ready(function() {
	...
	bulkRestoreBtn.on('click', function() {
		...
		Swal.fire({
			title: 'Restore Parents?',
			html: `You are about to restore <strong>${ids.length}</strong> parent(s):<br><br><small>${namesText}</small>`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.parents.bulk-restore") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						parent_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Restoring...',
							text: 'Please wait while we restore the selected parents.',
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>${ids.length}</strong> parent(s):<br><br><small>${namesText}</small>`,
			icon: 'warning',
			showCancelButton: true,
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.parents.bulk-force-delete") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						parent_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Deleting...',
							text: 'Please wait while we permanently delete the selected parents.',
							allowOutsideClick: false,
							allowEscapeKey: false,
							didOpen: () => {
								Swal.showLoading();
	...
	jQuery('.force-delete-btn').on('click', function() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: `/admin/parents/${id}/force-delete`,
					type: 'DELETE',
					data: {
						_token: '{{ csrf_token() }}'
					},
	...
	jQuery('#emptyTrashBtn').on('click', function() {
		Swal.fire({
			title: 'Empty Trash?',
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>all {{ $trashedCount }} parent(s)</strong> in the trash.`,
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d33',
			cancelButtonColor: '#6c757d',
