...
@section('title', 'Students Trash')

@section('page-title', 'Students Trash')
		...
		<div class="card">
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					<h5>Deleted Students ({{ $trashedCount }})</h5>
			...
			<div class="card-body">
				<!-- Search -->
				<form method="GET" action="{{ route('admin.students.trash') }}" class="mb-4">
					<div class="row g-3">
						<div class="col-md-4">
						...
						@if(request('search'))
							<div class="col-md-1">
								<a href="{{ route('admin.students.trash') }}" class="btn btn-outline-secondary w-100" title="Clear">
									<i data-feather="x"></i>
								</a>
							</div>
						@endif
					...
					<table class="table table-striped table-hover">
						...
						<tbody>
							@forelse($students as $student)
								<tr>
									<td>
										<input type="checkbox" class="form-check-input student-checkbox" value="{{ $student->id }}" data-name="{{ $student->full_name }}">
									</td>
									...
									<td>
										<span class="text-muted" title="{{ $student->deleted_at->format('d M Y, h:i A') }}">
									...
									<td>
										<div class="common-align gap-2 justify-content-start">
											...
											<button type="button" class="square-white border-0 bg-transparent p-0 force-delete-btn" title="Delete Permanently" data-id="{{ $student->id }}" data-name="{{ $student->full_name }}">
...
<form id="emptyTrashForm" action="{{ route('admin.students.empty-trash') }}" method="POST" class="d-none">
	@csrf
	@method('DELETE')
</form>
@endsection
...
jQuery(document).ready(function() {
	...
	const studentCheckboxes = jQuery('.student-checkbox');
	...
	function updateBulkState() {
		const checkedCount = jQuery('.student-checkbox:checked').length;
	...
	selectAllCheckbox.on('change', function() {
		studentCheckboxes.prop('checked', jQuery(this).is(':checked'));
		updateBulkState();
	});
	...
	function getSelectedData() {
		...
		jQuery('.student-checkbox:checked').each(function() {
	...
	bulkRestoreBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `You are about to restore <strong>${ids.length}</strong> student(s):<br><br><small>${namesText}</small>`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.students.bulk-restore") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						student_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Restoring...',
							text: 'Please wait while we restore the selected students.',
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>${ids.length}</strong> student(s):<br><br><small>${namesText}</small>`,
			icon: 'warning',
			showCancelButton: true,
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.students.bulk-force-delete") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						student_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Deleting...',
							text: 'Please wait while we permanently delete the selected students.',
							allowOutsideClick: false,
							allowEscapeKey: false,
							didOpen: () => {
								Swal.showLoading();
	...
	jQuery('#emptyTrashBtn').on('click', function() {
		Swal.fire({
			title: 'Empty Trash?',
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>all {{ $trashedCount }} student(s)</strong> in the trash.`,
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d33',
			cancelButtonColor: '#6c757d',
