			...
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					...
					<div class="d-flex gap-2">
						...
						<a href="{{ route('admin.students.trash') }}" class="btn btn-outline-danger position-relative">
									...
									<td>
										<div class="common-align gap-2 justify-content-start">
											...
											<form action="{{ route('admin.students.destroy', $student) }}" method="POST" class="d-inline delete-form">
												@csrf
												@method('DELETE')
												<button type="button" class="square-white trash-7 border-0 bg-transparent p-0 move-to-trash" title="Move to Trash" data-name="{{ $student->full_name }}">
...
<script>
	...
	jQuery(document).ready(function() {
		...
		bulkDeleteBtn.on('click', function() {
			...
			Swal.fire({
				title: 'Move to Trash?',
				html: `You are about to move <strong>${selectedIds.length}</strong> student(s) to trash:<br><br><small>${namesText}</small><br><br><small class="text-muted">You can restore them later from the trash.</small>`,
				icon: 'warning',
					...
					jQuery.ajax({
						...
						beforeSend: function() {
							Swal.fire({
								...
								text: 'Please wait while we move the selected students to trash.',
		...
		jQuery(document).on('click', '.move-to-trash', function(e) {
			...
			var itemName = jQuery(this).data('name') || 'this student';

			Swal.fire({
				title: 'Move to Trash?',
				html: `You are about to move <strong>${itemName}</strong> to trash.<br><small class="text-muted">You can restore this student later from the trash.</small>`,
				icon: 'warning',
				showCancelButton: true,
