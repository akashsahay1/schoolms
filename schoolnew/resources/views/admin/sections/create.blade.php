						...
						<div class="col-md-6">
							...
							<select class="form-select @error('class_teacher_id') is-invalid @enderror" id="class_teacher_id" name="class_teacher_id">
								...
								@foreach($teachers as $teacher)
									...
									@endphp
									<option value="{{ $teacher->id }}" {{ old('class_teacher_id') == $teacher->id ? 'selected' : '' }} {{ $isAssigned ? 'disabled' : '' }} data-assigned="{{ $isAssigned ? '1' : '0' }}" data-assigned-class="{{ $assignedClass }}">
	...
	jQuery('#class_id').on('change', function() {
		...
		jQuery.ajax({
			...
			success: function(assigned) {
				...
				// If currently selected teacher is now assigned, reset selection
