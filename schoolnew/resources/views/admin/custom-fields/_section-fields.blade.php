{{--
    Renders custom fields that belong to a specific form section.
    Include this inside any form section card's row.

    Variables:
    - $customFields: Collection of active CustomField models
    - $customFieldValues: Array of [custom_field_id => value] (for edit forms)
    - $sectionKey: string — the section key to filter by (e.g., 'student_information')
--}}

@php
    $sectionFields = ($customFields ?? collect())->filter(function($field) use ($sectionKey) {
        return ($field->section ?? 'additional_information') === $sectionKey;
    });
@endphp
