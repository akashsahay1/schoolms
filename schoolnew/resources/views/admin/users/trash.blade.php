...
@section('page-title', 'Users Trash')
		...
		<div class="card">
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					<h5>Deleted Users ({{ $trashedCount }})</h5>
			...
			<div class="card-body">
				<!-- Search -->
				<form method="GET" action="{{ route('admin.users.trash') }}" class="mb-4">
					<div class="row g-3">
						<div class="col-md-4">
						...
						@if(request('search'))
							<div class="col-md-1">
								<a href="{{ route('admin.users.trash') }}" class="btn btn-outline-secondary w-100" title="Clear">
									<i data-feather="x"></i>
								</a>
							</div>
						@endif
						...
						<tbody>
							@forelse($users as $user)
								<tr>
									...
									<td>
										<span class="text-muted" title="{{ $user->deleted_at->format('d M Y, h:i A') }}">
									...
									<td>
										<div class="common-align gap-2 justify-content-start">
											...
											<button type="button" class="square-white border-0 bg-transparent p-0 force-delete-btn" title="Delete Permanently" data-id="{{ $user->id }}" data-name="{{ $user->name }}">
...
<form id="emptyTrashForm" action="{{ route('admin.users.empty-trash') }}" method="POST" class="d-none">
	@csrf
	@method('DELETE')
</form>
@endsection
...
jQuery(document).ready(function() {
	...
	bulkRestoreBtn.on('click', function() {
		...
		Swal.fire({
			title: 'Restore Users?',
			html: `You are about to restore <strong>${ids.length}</strong> user(s):<br><br><small>${namesText}</small>`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Restoring...',
							text: 'Please wait while we restore the selected users.',
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>${ids.length}</strong> user(s):<br><br><small>${namesText}</small>`,
			icon: 'warning',
			showCancelButton: true,
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Deleting...',
							text: 'Please wait while we permanently delete the selected users.',
							allowOutsideClick: false,
							allowEscapeKey: false,
							didOpen: () => {
								Swal.showLoading();
	...
	jQuery('#emptyTrashBtn').on('click', function() {
		Swal.fire({
			title: 'Empty Trash?',
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>all {{ $trashedCount }} user(s)</strong> in the trash.`,
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d33',
			cancelButtonColor: '#6c757d',
