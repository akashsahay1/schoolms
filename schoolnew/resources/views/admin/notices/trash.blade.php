...
@section('title', 'Notices Trash')

@section('page-title', 'Notices Trash')
		...
		<div class="card">
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					<h5>Deleted Notices ({{ $trashedCount }})</h5>
					<div class="d-flex gap-2">
						...
						@endif
						<a href="{{ route('admin.notices.index') }}" class="btn btn-outline-primary">
							<i data-feather="arrow-left" class="me-1"></i> Back to Notices
						</a>
					</div>
				</div>
			</div>
			<div class="card-body">
				<!-- Search -->
				<form method="GET" action="{{ route('admin.notices.trash') }}" class="mb-4">
					<div class="row g-3">
						<div class="col-md-4">
						...
						@if(request('search'))
							<div class="col-md-1">
								<a href="{{ route('admin.notices.trash') }}" class="btn btn-outline-secondary w-100" title="Clear">
									<i data-feather="x"></i>
								</a>
							</div>
						@endif
					...
					<table class="table table-striped table-hover">
						...
						<tbody>
							@forelse($notices as $notice)
								<tr>
									<td>
										<input type="checkbox" class="form-check-input notice-checkbox" value="{{ $notice->id }}" data-title="{{ $notice->title }}">
									</td>
									<td>{{ $notices->firstItem() + $loop->index }}</td>
									<td>{{ $notice->title }}</td>
									<td>
										<span class="badge badge-light-{{ $notice->type === 'urgent' ? 'danger' : ($notice->type === 'important' ? 'warning' : 'info') }}">
											{{ ucfirst($notice->type ?? 'General') }}
										</span>
									</td>
									<td>
										<span class="text-muted" title="{{ $notice->deleted_at->format('d M Y, h:i A') }}">
											{{ $notice->deleted_at->diffForHumans() }}
										</span>
									</td>
									<td>
										<div class="common-align gap-2 justify-content-start">
											<form action="{{ route('admin.notices.restore', $notice->id) }}" method="POST" class="d-inline">
												@csrf
											...
											<button type="button" class="square-white border-0 bg-transparent p-0 force-delete-btn" title="Delete Permanently" data-id="{{ $notice->id }}" data-title="{{ $notice->title }}">
...
<form id="emptyTrashForm" action="{{ route('admin.notices.empty-trash') }}" method="POST" class="d-none">
	@csrf
	@method('DELETE')
</form>
@endsection
...
jQuery(document).ready(function() {
	...
	bulkRestoreBtn.on('click', function() {
		...
		Swal.fire({
			title: 'Restore Notices?',
			html: `You are about to restore <strong>${ids.length}</strong> notice(s):<br><br><small>${titlesText}</small>`,
			icon: 'question',
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.notices.bulk-restore") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						notice_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Restoring...',
							text: 'Please wait while we restore the selected notices.',
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			...
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>${ids.length}</strong> notice(s):<br><br><small>${titlesText}</small>`,
			icon: 'warning',
			showCancelButton: true,
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: '{{ route("admin.notices.bulk-force-delete") }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						notice_ids: ids
					...
					beforeSend: function() {
						Swal.fire({
							title: 'Deleting...',
							text: 'Please wait while we permanently delete the selected notices.',
							allowOutsideClick: false,
							allowEscapeKey: false,
							didOpen: () => {
								Swal.showLoading();
	...
	jQuery('.force-delete-btn').on('click', function() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				jQuery.ajax({
					url: `/admin/notices/${id}/force-delete`,
					type: 'DELETE',
					data: {
						_token: '{{ csrf_token() }}'
					},
	...
	jQuery('#emptyTrashBtn').on('click', function() {
		Swal.fire({
			title: 'Empty Trash?',
			html: `<div class="text-danger"><strong>Warning: This action cannot be undone!</strong></div><br>You are about to permanently delete <strong>all {{ $trashedCount }} notice(s)</strong> in the trash.`,
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#d33',
			cancelButtonColor: '#6c757d',
