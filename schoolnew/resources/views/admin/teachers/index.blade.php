			...
			<div class="card-header">
				<div class="d-flex justify-content-between align-items-center">
					...
					<div class="d-flex gap-2">
						...
						<a href="{{ route('admin.teachers.trash') }}" class="btn btn-outline-danger position-relative">
								...
								<tr>
									...
									<td>
										<div class="common-align gap-2 justify-content-start">
											<a class="square-white" href="{{ route('admin.teachers.show', $teacher) }}" title="View">
												<svg><use href="{{ asset('assets/svg/icon-sprite.svg#eye') }}"></use></svg>
											</a>
											<a class="square-white" href="{{ route('admin.teachers.edit', $teacher) }}" title="Edit">
												<svg><use href="{{ asset('assets/svg/icon-sprite.svg#edit-content') }}"></use></svg>
											</a>
											<form action="{{ route('admin.teachers.destroy', $teacher) }}" method="POST" class="d-inline delete-form">
												@csrf
												@method('DELETE')
												<button type="button" class="square-white trash-7 border-0 bg-transparent p-0 move-to-trash" title="Move to Trash" data-name="{{ $teacher->full_name }}">
								...
								<tr>
									<td colspan="10" class="text-center py-4">
										<div class="text-muted">
											...
											<a href="{{ route('admin.teachers.create') }}" class="btn btn-primary mt-3">Add First Teacher</a>
										</div>
									</td>
								</tr>
							@endforelse
...
jQuery(document).ready(function() {
	...
	bulkDeleteBtn.on('click', function() {
		...
		Swal.fire({
			title: 'Move to Trash?',
			html: `You are about to move <strong>${selectedIds.length}</strong> teacher(s) to trash:<br><br><small>${namesText}</small><br><br><small class="text-muted">You can restore them later from the trash.</small>`,
			icon: 'warning',
				...
				jQuery.ajax({
					...
					beforeSend: function() {
						Swal.fire({
							...
							text: 'Please wait while we move the selected teachers to trash.',
	...
	jQuery(document).on('click', '.move-to-trash', function(e) {
		...
		var itemName = jQuery(this).data('name') || 'this teacher';

		Swal.fire({
			title: 'Move to Trash?',
			html: `You are about to move <strong>${itemName}</strong> to trash.<br><small class="text-muted">You can restore this teacher later from the trash.</small>`,
			icon: 'warning',
			showCancelButton: true,
