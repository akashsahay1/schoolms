...
(function () {
  ...
  if (document.querySelector("#inline-calender2")) {
  ...
  if (document.querySelector("#chart_current_academic")) {
  ...
  // Student Chart - Use real data from PHP
  var maleCount = window.dashboardData ? window.dashboardData.genderStats.male : 0;
  var femaleCount = window.dashboardData ? window.dashboardData.genderStats.female : 0;
  var otherCount = window.dashboardData ? window.dashboardData.genderStats.other : 0;
  var totalStudents = window.dashboardData ? window.dashboardData.totalStudents : 0;

  // Build series and labels dynamically (only include categories with data)
  var studentSeries = [];
  var studentLabels = [];
  var studentColors = [];

  if (maleCount > 0 || (maleCount === 0 && femaleCount === 0 && otherCount === 0)) {
    studentSeries.push(maleCount);
    studentLabels.push("Boys");
    studentColors.push("#7366FF");
  }
  if (femaleCount > 0 || (maleCount === 0 && femaleCount === 0 && otherCount === 0)) {
    studentSeries.push(femaleCount);
    studentLabels.push("Girls");
    studentColors.push("#ffb829");
  }
  if (otherCount > 0) {
    studentSeries.push(otherCount);
    studentLabels.push("Other");
    studentColors.push("#54BA4A");
  }

  // If no students, show placeholder
  if (studentSeries.length === 0) {
    studentSeries = [0, 0];
    studentLabels = ["Boys", "Girls"];
    studentColors = ["#7366FF", "#ffb829"];
  }
  ...
  if (document.querySelector("#student-chart")) {
    window.studentChartInstance = new ApexCharts(document.querySelector("#student-chart"), totalStudentOption);
